...
const Index = () => {
  ...
  const { search, category, filterPrice, filterCapacity } =
        ...
        {places?.length > 0 ? (
          places.map((place) => (
            <div key={place._id} className="relative">
              ...
              <div
                className="absolute right-2 bottom-4 hover:text-red-600"
          ...
          <div
            className="fixed text-2xl text-center mt-10"
            style={{ width: "94%" }}
          >
          ...
          <div className="absolute top-1/3 w-full transition-opacity">
            <div className="mx-auto w-1/3 backdrop-filter backdrop-blur-md bg-opacity-25 p-2 font-semibold rounded-md text-xl border-primary border">
              <div className="ml-2 mt-2">
                ...
                <span className="text-primary">{placeToDelete?.title}</span>"?
              </div>
              <div className="flex justify-between p-2">
                ...
                <button
                  className="p-2 hover:text-white rounded-md"
                  onClick={() => {
                    setDeleteDialog(false);
